...
import { PencilIcon, SparklesIcon, PlusIcon, TrashIcon } from './icons';
            ...
            {!plan ? (
                ...
                <div className="flex-grow p-4 flex flex-col overflow-y-hidden">
                    ...
                    <div className="flex-grow mt-4 overflow-y-auto pr-2">
                        ...
                        {activeTab === 'characters' && <CharactersEditor characters={plan.characterSettings} onChange={handleCharacterChange} onAdd={handleAddCharacter} onDelete={handleDeleteCharacter} disabled={disabled} />}
        ...
        <div className="border border-gray-200 dark:border-gray-700 rounded-md mb-2">
            <h2>
                <button type="button" onClick={() => setIsOpen(!isOpen)} disabled={disabled} className="flex items-center justify-between w-full p-3 font-medium text-left text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800" aria-expanded={isOpen}>
                    <input type="text" value={title} onChange={e => onTitleChange(e.target.value)} disabled={disabled} className="bg-transparent font-semibold focus:ring-1 ring-indigo-500 rounded-sm w-full mr-2"/>
                    <div className='flex items-center'>
                         <button onClick={onDelete} disabled={disabled} className="p-1 text-gray-400 hover:text-red-500 disabled:text-gray-600"><TrashIcon /></button>
...
const CharactersEditor: React.FC<{characters: CharacterProfile[], onChange: (id: string, field: keyof Omit<CharacterProfile, 'id'>, value: string) => void, onAdd: () => void, onDelete: (id: string) => void, disabled: boolean}> = ({ characters, onChange, onAdd, onDelete, disabled }) => (
    <div>
        {characters.map(char => (
